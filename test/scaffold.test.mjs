...
const requiredApplicationFiles = ["src/app/globals.css", "src/app/layout.tsx", "src/app/page.tsx"];
...
test("the package contract exposes the required verification commands", async () => {
  ...
  assert.deepEqual(
    ["dev", "lint", "format:check", "typecheck", "test", "build", "verify"].filter(
      (script) => !packageJson.scripts?.[script],
    ),
    [],
  );
  assert.match(packageJson.scripts.verify, /npm run format:check/);
