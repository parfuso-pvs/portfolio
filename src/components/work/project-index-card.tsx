...
function MemxArtifact() {
  return (
    <div className="relative h-full min-h-64 overflow-hidden" aria-hidden="true">
      <div className="material-blueprint absolute inset-0" />
      <div className="border-accent/30 bg-paper/82 absolute top-[16%] left-[13%] h-[62%] w-[64%] border shadow-sheet transition-transform duration-500 ease-out group-hover:-translate-y-1 group-hover:translate-x-1">
      ...
      <div className="border-accent/45 bg-paper-raised/88 absolute right-[11%] bottom-[12%] h-[46%] w-[56%] border shadow-pinned transition-transform duration-500 ease-out group-hover:translate-y-1 group-hover:-translate-x-1">
        ...
        <span className="type-mono text-accent absolute top-4 left-4">08 / markets</span>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-7 left-7">
        <span className="type-mono text-media-foreground/60">Tomorrow / intentional</span>
        <p className="mt-3 max-w-44 font-serif text-3xl leading-[0.95] tracking-[-0.035em]">
          Plan the day before it begins.
        </p>
      </div>
      <div className="bg-paper text-ink absolute right-[10%] bottom-0 h-[68%] w-[42%] min-w-32 rounded-t-[1.75rem] border-[5px] border-b-0 border-ink px-4 pt-7 shadow-pinned transition-transform duration-500 ease-out group-hover:-translate-y-2">
        <span className="type-label text-accent">Top priority</span>
        <div className="border-line mt-4 border-t pt-3">
          <span className="type-mono text-muted">One clear thing</span>
        </div>
...
function IffersArtifact() {
  return (
    <div className="bg-paper-deep relative h-full min-h-64 overflow-hidden" aria-hidden="true">
      ...
      <div className="border-line-strong bg-paper-raised absolute top-[14%] bottom-[12%] left-[9%] w-[58%] border shadow-pinned transition-transform duration-500 ease-out group-hover:-rotate-1 group-hover:scale-[1.015]">
        ...
        <span className="type-mono text-muted absolute bottom-3 left-3">
          Light / composition / story
        </span>
      </div>
      <div className="material-vellum absolute right-[8%] bottom-[9%] w-[38%] p-4 shadow-sheet">
        <span className="type-label text-accent">Full ownership</span>
        <span className="type-mono text-ink mt-2 block">Copy / design / code</span>
      </div>
    </div>
  );
}

function ProjectArtifact({ projectId }: { projectId: FeaturedProjectRecord["id"] }) {
  switch (projectId) {
    case "memx":
      return <MemxArtifact />;
    case "domani":
      return <DomaniArtifact />;
    case "iffers-pictures":
      return <IffersArtifact />;
  }
}
...
export function ProjectIndexCard({ project, sequence }: ProjectIndexCardProps) {
  return (
    ...
    >
      ...
      >
        <ProjectArtifact projectId={project.id} />
        <div className="px-sheet-inset relative border-t border-line py-7 sm:py-8">
          <div className="flex items-start justify-between gap-6">
            ...
            <span className="text-accent mt-1 grid size-11 shrink-0 place-items-center border border-accent transition-transform duration-300 ease-out group-hover:translate-x-1 group-hover:-translate-y-1 group-active:scale-[0.96]">
              <ArrowUpRight />
            </span>
          </div>
